        ...
        '<div class="modal-dialog" role="document">' + 
          '<div class="modal-content">' + 
            '<div class="modal-header">' + 
              ...
              '<h5 id="' + affiliationModalId + '-title" class="modal-title">Search for organization in ISNI</h5>' + 
            '</div>' + 
...
function affiliationsQuery(str) {
  ...
  fetch("https://isni.ringgold.com/api/stable/search?q=" + str)
  ...
  .then(data => {
    ...
    data.items.forEach((doc) => {
      ...
      let label = doc.name != null ? doc.name : '';
      let address = ( doc.locality != null ? doc.locality : '' ) + ( doc.country_code!= null ? ( ', ' + doc.country_code ) : '');
      // Add a table row for the doc
      table.innerHTML += 
      '<tr title="' + label + '">' +
        '<td><a href="' + doc.isni + '" target="_blank">' + doc.name + '</a></td>' +
        '<td>' + address + '</td>' +
        '<td>' + 
          '<span ' + 
...
var authorSelector = "div ~ div#metadata_author ~ div.dataset-field-values div.edit-compound-field";
